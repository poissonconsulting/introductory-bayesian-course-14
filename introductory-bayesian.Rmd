...
The population of house martins in a small Swiss village has been surveyed by Reto Freuler since 1990

The `jaggernaut` package includes data sets from kery and Schaub's  excellent book Population Analysis using WinBUGS. Here we consider 
the annual surveys of house martins in a small swiss village from 1990 to 2009.
...
The following BUGS code uses a state-space population growth 
model to estimate the probability that the population size in 2015 would be lower than in 2009. 

Algebraically the state space model can be written 

$log(N_{t+1}) = log(N_{t}) + r_{t}$ where $r_{t}$ is the the stochastic population growth rate and$r_{t} \sim N(\bar{r}, \sigma_{r})$
...
mhm1 <- jags_model("model {
  ...
  for (i in 1:length(year)) {
    r[i] ~ dnorm(mean.r, sigma.proc^-2)
    logN.est[i+1] <- logN.est[i] + r[i]
    hm[i] ~ dlnorm(logN.est[i], sigma.obs^-2)
  }
}",
derived_code = "model{
  for (i in 1:length(year)) {
