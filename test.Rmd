...
```{r, message=FALSE, warning=FALSE, results='hide'}
analysis1 <- jags_analysis(model1, data = trees, niters = 10^5)
```
...
relative to 0.5 mg of Vitamin C.
...
```{r, echo = FALSE, results = 'asis'}
exercise("Plot the percent change in `len` for all four models relative to 1 mg of Orange Juice")
```

# Peregrine Falcon Population

Consider the peregrine falcon population data `?peregrine`.
the following code regresses `Pairs` on `Year`.
...
fits a second-order polynomial on `Pairs`
